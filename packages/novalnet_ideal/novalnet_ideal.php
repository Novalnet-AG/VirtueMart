 ...
 * @since   11.1
 */
class plgVmPaymentnovalnet_ideal extends vmPSPlugin
{
    /**
    ...
    public function __construct(&$subject, $config)
    {
        ...
        $this->helpers->loadNovalnetJs('novalnet_ideal', 'admin');
    }
    ...
    {
        return $this->createTableSQL('Novalnet iDeal payment Table');
    }
    ...
    public function getTableSQLFields()
    {
        ...
        if (empty($table['paid_amount']))
                $table['paid_amount'] = 'int(11) UNSIGNED COMMENT \'Customer paid amount in cents\'';
         return $table;
    }
    ...
    public function plgVmConfirmedOrder($cart, $order)
    {
        ...
        $this->helpers->handleSession('novalnet_ideal', 'set', 'payment_request', $data);
        $html                 = $this->helpers->buildRedirectform($data, 'novalnet_ideal');
    ...
    public function plgVmOnPaymentResponseReceived(&$html)
    {
        ...
        {
            $hash2 = $this->helpers->generateHashValue($response);
            if (($hash2 != $response['hash2']))
                    $this->helpers->handleMessage(vmText::_('VMPAYMENT_NOVALNET_CHECK_HASH_FAILED_ERROR'));
